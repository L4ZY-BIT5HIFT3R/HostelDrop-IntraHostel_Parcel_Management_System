...
def load_server_module(
    *,
    ...
    jwt_secret: str | None = "test-secret",
    admin_password: str | None = "test-admin-password",
):
...
def test_default_app_env_is_production():
    server = load_server_module(app_env=None)
...
def test_production_rejects_missing_jwt_secret():
    with pytest.raises(RuntimeError, match="JWT_SECRET_KEY must be set in production"):
        load_server_module(app_env="production", jwt_secret=None, admin_password="set-admin-password")
